...
public class MainActivity extends Activity {
    ...
    void launchGame() {
        ...
        if (intent != null) {
            Toast.makeText(this, "✅ Запускаем игру!", Toast.LENGTH_SHORT).show();
            startActivity(intent);
        } else {
            // Пробуем другие известные пакеты Black Russia
            String[] packages = {"com.blackrussia.game", "com.br.android", "com.blackrussia"};
            for (String pkg : packages) {
                intent = getPackageManager().getLaunchIntentForPackage(pkg);
                if (intent != null) {
                    Toast.makeText(this, "✅ Запускаем!", Toast.LENGTH_SHORT).show();
                    startActivity(intent);
                    return;
                }
            }
            Toast.makeText(this, "❌ Black Russia не установлена", Toast.LENGTH_LONG).show();
        }
    }
